...
Canonical procedure: `core/sdlc/architecture-review.md`. Read it before
running this skill.
...
`core/sdlc/architecture-review.md`.

## Process

1. Read `core/sdlc/architecture-review.md` in full.
...
6. If the project matches a known archetype, compare the repo's actual
   structure against `core/archetypes/<type>/structure.md` and `rules.md`
   — flag drift, don't mandate a restructure.
7. Compare the repo's actual boundaries against
   `core/standards/repository-architecture.md` for whether a
...
- `core/sdlc/architecture-review.md` — full process
- `core/archetypes/<type>/structure.md` and `rules.md` — stack-specific structure/rules to check alignment against, if applicable
- `core/standards/repository-architecture.md` — single-repo/monorepo/split decision criteria
- `devops-review` — infra/CI-specific detail handoff
- `rollback-plan` (`core/sdlc/rollback-plan.md`) — rollback procedure handoff
