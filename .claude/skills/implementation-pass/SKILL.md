...
Canonical procedure: `core/sdlc/implementation-pass.md`. Read it before
running this skill.
...
1. Read `core/sdlc/implementation-pass.md` in full and the feature folder.
2. Read the applicable `core/standards/*` files (e.g. `code-quality.md`,
   `security.md`, `testing.md`, `configuration.md`) and, if the project
   matches a known stack, its `core/archetypes/<type>/` files — not only
...
6. Update docs per `core/sdlc/docs-sync.md` if behavior/architecture/config/
...
- `core/sdlc/implementation-pass.md` — full process
- `core/sdlc/docs-sync.md` — docs-impact follow-up
- `core/standards/` — cross-cutting rules (code quality, security, testing,
  configuration, etc.) applicable regardless of feature; check before
  implementing
- `core/archetypes/<type>/` — stack-specific structure/rules/validation, if
